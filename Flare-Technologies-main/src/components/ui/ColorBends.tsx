...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uColorCount: { value: 0 }, uColors: { value: uColorsArray },
    ...
    const loop = () => {
      const dt = clock.getDelta(); const elapsed = clock.elapsedTime;
  ...
  useEffect(() => {
    ...
    m.uniforms.uSpeed.value = speed; m.uniforms.uScale.value = scale;
    m.uniforms.uFrequency.value = frequency; m.uniforms.uWarpStrength.value = warpStrength;
    m.uniforms.uMouseInfluence.value = mouseInfluence; m.uniforms.uParallax.value = parallax;
    m.uniforms.uNoise.value = noise; m.uniforms.uIterations.value = iterations;
    m.uniforms.uIntensity.value = intensity; m.uniforms.uBandWidth.value = bandWidth;
    ...
    for (let i = 0; i < MAX_COLORS; i++) {
      const v = m.uniforms.uColors.value[i];
      if (i < arr.length) v.copy(arr[i]); else v.set(0,0,0);
    }
