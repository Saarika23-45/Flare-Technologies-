...
function createTextTexture(
  ...
  // Background - Use the provided cardColor
  ...
  // Content - Use dark text for light/pastel backgrounds
  context.fillStyle = '#121620'; 
  context.textAlign = "center";
  context.textBaseline = "middle";

  // 1. Draw Title (with wrapping)
  const titleFont = `${font.split(' ').slice(2).join(' ')}`;
  context.font = `bold 65px ${titleFont}`;
  context.fillStyle = '#121620';
  
  const titleWords = text.toUpperCase().split(' ');
  let titleLine = '';
  let titleY = canvas.height / 2 - 120;
  ...
  // Adjust Y starting point based on number of lines
  titleY -= (titleLines.length - 1) * (titleLineHeight / 2);

  titleLines.forEach((line) => {
    context.fillText(line.trim(), canvas.width / 2, titleY);
    titleY += titleLineHeight;
  });

  // 2. Draw Description (with wrapping)
  if (description) {
    context.fillStyle = 'rgba(18, 22, 32, 0.7)';
    context.font = `300 44px ${titleFont}`;
    const words = description.split(' ');
    let line = '';
    let descriptionY = titleY + 20; // Start description after title lines
    const maxWidth = 800;
    const lineHeight = 60;
    ...
    for (let n = 0; n < words.length; n++) {
      ...
      if (testWidth > maxWidth && n > 0) {
        context.fillText(line, canvas.width / 2, descriptionY);
        line = words[n] + ' ';
...
class Media {
  ...
  onResize(
    ...
    this.plane.scale.x =
      (this.viewport.width * (850 * this.scale)) / this.screen.width;
...
class App {
  ...
  addEventListeners() {
    ...
    this.container.addEventListener("touchstart", this.boundOnTouchDown as any);
